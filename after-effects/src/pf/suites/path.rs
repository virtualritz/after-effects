...
impl PathQuerySuite {
    ...
    pub fn checkout_path(&self, effect_ref: impl AsPtr<PF_ProgPtr>, unique_id: PF_PathID, what_time: i32, time_step: i32, time_scale: u32) -> Result<PF_PathOutlinePtr, Error> {
        call_suite_fn_single!(self, PF_CheckoutPath -> PF_PathOutlinePtr, effect_ref.as_ptr(), unique_id, what_time, time_step, time_scale)
    }

    /// Releases the path back to After Effects. Always do this, regardless of any error conditions
    /// encountered. Every checkout must be balanced by a checkin, or pain will ensue.
