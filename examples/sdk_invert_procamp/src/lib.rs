...
unsafe extern "C" fn dispose_data(d: *mut std::ffi::c_void) {
    let _ = Box::<InvertProcAmpParams>::from_raw(d as _);
}
unsafe fn pre_render(
    in_data: ae::pf::InDataHandle,
    extra: *mut ae_sys::PF_PreRenderExtra,
) -> ae_sys::PF_Err {
    ...
    if let Ok(in_result) = pre_render_cb.checkout_layer(
        in_data.effect_ref(),
        PluginParams::InputLayer as i32,
        0,
        &req,
        ...
        union_rect(
            &in_result.max_result_rect,
            &mut (*(*extra).output).max_result_rect,
        );
    }
...
fn smart_render_cpu(
    ...
    input_world: ae::EffectWorld,
    output_world: ae::EffectWorld,
    info: *const InvertProcAmpParams,
) -> ae_sys::PF_Err {
    match pixel_format {
            ...
            if let Ok(suite) = ae::pf::IterateFloatSuite::new() {
                suite
                    .iterate(
                        in_data,
                        0,
                        output_world.height() as i32,
                        input_world,
                        None,
                        info as *const _,
                        Some(filter_image_cpu::filter_image_32),
                        output_world,
                    )
                    .unwrap();
            }
            ...
            if let Ok(suite) = ae::pf::Iterate16Suite::new() {
                suite
                    .iterate(
                        ...
                        None,
                        info as *const _,
                        Some(filter_image_cpu::filter_image_16),
                        output_world,
                    )
                    .unwrap();
            }
            ...
            if let Ok(suite) = ae::pf::Iterate8Suite::new() {
                suite
                    .iterate(
                        ...
                        None,
                        info as *const _,
                        Some(filter_image_cpu::filter_image_8),
                        output_world,
                    )
                    .unwrap();
            }
...
unsafe fn smart_render(
    in_data: ae::pf::InDataHandle,
    extra: *mut ae_sys::PF_SmartRenderExtra,
    is_gpu: bool,
) -> ae_sys::PF_Err {
    ...
    if !info.is_null() {
        ...
        let input_world = cb
            .checkout_layer_pixels(in_data.effect_ref(), PluginParams::InputLayer as u32)
            .unwrap();
        ...
        cb.checkin_layer_pixels(in_data.effect_ref(), PluginParams::InputLayer as u32)
            .unwrap();
...
impl Default for OpenCLGPUData {
    fn default() -> Self {
        Self {
            invert_kernel: std::ptr::null_mut(),
            procamp_kernel: std::ptr::null_mut(),
        }
    }
}
struct MetalGPUData {
...
impl Default for MetalGPUData {
    fn default() -> Self {
        Self {
            invert_pipeline: std::ptr::null_mut(),
            procamp_pipeline: std::ptr::null_mut(),
        }
    }
}

unsafe fn gpu_device_setup(
    in_data: ae::pf::InDataHandle,
    out_data: *mut ae_sys::PF_OutData,
    extra: *mut ae_sys::PF_GPUDeviceSetupExtra,
) -> ae_sys::PF_Err {
    let device_info = ae::pf::GPUDeviceSuite1::new()
        .unwrap()
        .get_device_info(in_data, (*(*extra).input).device_index)
        .unwrap();
    ...
    match what_gpu {
        ...
        ae_sys::PF_GPU_Framework_OPENCL => {
            ...
            if result != cl_sys::CL_SUCCESS {
                log::debug!("clCreateProgramWithSource failed: {result}");
            }

            result = cl_sys::clBuildProgram(
                program,
                1,
                &device,
                cstr!("-cl-single-precision-constant -cl-fast-relaxed-math").as_ptr(),
                None,
                std::ptr::null_mut(),
            );
            if result != cl_sys::CL_SUCCESS {
                log::debug!("clBuildProgram failed: {result}");
            }
            gpu_data.invert_kernel =
                cl_sys::clCreateKernel(program, cstr!("InvertColorKernel").as_ptr(), &mut result);
            if result != cl_sys::CL_SUCCESS {
                log::debug!("clCreateKernel failed: {result}");
            }
            gpu_data.procamp_kernel =
                cl_sys::clCreateKernel(program, cstr!("ProcAmp2Kernel").as_ptr(), &mut result);
            if result != cl_sys::CL_SUCCESS {
                log::debug!("clCreateKernel failed: {result}");
            }

            (*(*extra).output).gpu_data =
                ae::pf::Handle::into_raw(ae::pf::Handle::new(gpu_data).unwrap()) as *mut _;
...
unsafe fn gpu_device_setdown(
    in_data: ae::pf::InDataHandle,
    out_data: *mut ae_sys::PF_OutData,
    extra: *mut ae_sys::PF_GPUDeviceSetdownExtra,
) -> ae_sys::PF_Err {
    let what_gpu = (*(*extra).input).what_gpu;
    ...
    if what_gpu == ae_sys::PF_GPU_Framework_OPENCL {
        let mut gpu_data_handle = ae::pf::Handle::<OpenCLGPUData>::from_raw(
            (*(*extra).input).gpu_data as ae_sys::PF_Handle,
        )
        .unwrap();
        let gpu_data = gpu_data_handle.lock().unwrap();
        log::info!(
            "gpu_data.invert_kernel: {:?}",
            gpu_data.as_ref().unwrap().invert_kernel
        );
    ...
    match cmd as ae::EnumIntType {
        ...
        ae_sys::PF_Cmd_GLOBAL_SETUP => {
            ...
            if &in_data.application_id() == b"PrMr" {
                ...
                pixel_format
                    .clear_supported_pixel_formats(in_data.effect_ref())
                    .unwrap();
                pixel_format
                    .add_supported_pixel_format(
                        in_data.effect_ref(),
                        ae_sys::PrPixelFormat_PrPixelFormat_VUYA_4444_32f,
                    )
                    .unwrap();
            } else {
                (*out_data).out_flags2 |= ae_sys::PF_OutFlag2_SUPPORTS_GPU_RENDER_F32;
        ...
        ae_sys::PF_Cmd_PARAMS_SETUP => {
            ...
            ParamDef::new(in_data)
                .name("Hue")
                .param(Param::Angle(*AngleDef::new().set_value(0)))
                .add(-1);
        ...
        ae_sys::PF_Cmd_GPU_DEVICE_SETUP => {
            err = gpu_device_setup(
                in_data,
                out_data,
                extra as *mut ae_sys::PF_GPUDeviceSetupExtra,
            );
        }
        ae_sys::PF_Cmd_GPU_DEVICE_SETDOWN => {
            err = gpu_device_setdown(
                in_data,
                out_data,
                extra as *mut ae_sys::PF_GPUDeviceSetdownExtra,
            );
        }
